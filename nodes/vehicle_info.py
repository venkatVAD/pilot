...
class Main_variables():
    def __init__(self):
        ...
        self.throttle_voltage={'status' : None ,"data" : None,'time' : None}
        ...
        self.drive_break={'status' : None ,"data" : None,'time' : None}
        self.handbreak={'status' : None ,"data" : None,'time' : None}
...
class Motor(Thread,Main_variables,Can_connection):
    ...
    def f6(self):
        while True:
            msg10=self.bus.recv()
            if msg10.arbitration_id==0x188:
                f_break=bytearray(msg10.data)
                he= f_break.hex()

                oq= he[4:6]
                self.drive_break= int(oq,16)
                self.drive_break['status']=True
                self.drive_break['data']=self.drive_break
                self.drive_dirctn['time']=time()

                
                gq= self.to_little(self.he[0:4])
                throttle=int(gq,16)
                thro_voltage=throttle/256
                self.throttle_voltage['status']=True
                self.throttle_voltage['data']=thro_voltage
                self.throttle_voltage['time']=time()
    ...
    def batteryf2(self):
        while True:
            ...
            if msg6.arbitration_id==0x183:
                a=bytearray(msg6.data)
                he=self.a.hex() #converting bytearray to hex format

                _as= self.to_little(he[0:2])
                self.batterysoc=int(_as,16)*0.5
                self.batterysoc['status']=True
                self.batterysoc['data']=self.batterysoc
                self.batterysoc['time']=time()
                at= self.to_little(he[2:6])
                ...
                av=self.to_little(he[8:10])
                self.highesttemperaturethermistorid=int(av,16)*1
                
                aw=self.to_little(he[10:12])
                ...
                ax=self.to_little(he[12:14])
                self.highestlowestthermistorid=int(ax,16)*1

                ay=self.to_little(he[14:16])
    ...
    def batteryf3(self):
        while True:
            ...
            if msg7.arbitration_id==0x184:
                a=bytearray(msg7.data)
                he=a.hex()

                az= self.to_little(he[0:2])   
...
if __name__=="__main__":
    ...
    while True:
        ...
        print('throttle voltage',m.avg_Stater_Crnt)
